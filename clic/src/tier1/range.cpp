...
// Number of elements selected by a strided slice [start, stop) with the given step, following
// NumPy semantics: ceil(|stop - start| / |step|). The result is clamped to at least 1.
auto
range_size(int start, int stop, int step) -> size_t
{
  const int extent = std::abs(stop - start);
  const int stride = std::max(std::abs(step), 1);
  return std::max<size_t>((extent + stride - 1) / stride, 1);
}

// Gather kernel: reads a strided region of a large src image into a small contiguous dst image,
// following dst[i] = src[start + i * step]. The global range matches the dst size.
constexpr const char * gather_kernel_source = R"CLC(
...
// Scatter kernel: inverse of the gather kernel. It writes a small src image into a
...
{
  const size_t range_width = range_size(start_x, stop_x, step_x);
  const size_t range_height = range_size(start_y, stop_y, step_y);
  const size_t range_depth = range_size(start_z, stop_z, step_z);
  ...
  const KernelInfo    kernel = { "range", gather_kernel_source };
